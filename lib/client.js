...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var DICT_EN = {
			...
			"note.textTruncated": "truncated for display — use Copy for the full value",
		...
		var DICT_ZH = {
			...
			"note.textTruncated": "显示时已截断 —— 需要全文请用 Copy",
			...
			function renderTree() {
				...
				return react.createElement(
					...
					treeRows.map(function (row) {
						...
						if (row.kind === "text") {
							// Deliberately not a nested scroller: the pane owns the scroll, and
							// the header row above collapses this again.
							return react.createElement(
								"div",
								{
									key: row.path,
									style: { whiteSpace: "pre-wrap", wordBreak: "break-word", background: "#0d1117", borderLeft: "2px solid " + COLOR.line, margin: "2px 0 6px " + pad + "px", padding: "6px 10px" },
								},
								row.text,
								row.textTruncated ? react.createElement("div", { style: { color: COLOR.dim, fontSize: 12 } }, t("note.textTruncated")) : null,
							);
						}
						var rowChildren = [
