...
/** Longest string value rendered in full once expanded. */
export const EXPANDED_TEXT_LIMIT = 32768
 ...
 *
 * A string too long for its preview expands into one `text` row carrying the
 * value, capped at {@link EXPANDED_TEXT_LIMIT}; the clipped preview is a teaser,
 * and a message body or tool description is usually what the reader came for.
 *
 * @param root - the parsed JSON value.
...
function walk(value, path, label, depth, rows, expandedSet, sizes, limits, defaultLimit) {
  ...
  // A string too long for its preview is expandable too: a message body or a tool
  // description is usually the thing the reader came for.
  const longString = typeof value === 'string' && value.length > PREVIEW_CHARS
  const open = expandedSet.has(path)
  rows.push({
    path,
    depth,
    kind: kindOf(value),
    ...
    childCount: total,
    expandable: total > 0 || longString,
    expanded: (container || longString) && open,
  })
  if (!open) return
  if (longString) {
    rows.push({
      ...
      expandable: false,
      expanded: false,
      text: value.slice(0, EXPANDED_TEXT_LIMIT),
      textTruncated: value.length > EXPANDED_TEXT_LIMIT,
    })
    return
  }
  if (!container) return
